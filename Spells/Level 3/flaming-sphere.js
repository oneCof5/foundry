...
if (lastArg.tag === "OnUse") {
  ...
  const changeValue = `turn=end,saveDC=${caster.data.data.attributes.spelldc ?? 10},saveAbility=dex,damageRoll=${lastArg.spellLevel}d6,damageType=fire,saveDamage=halfdamage,saveRemove=false`;
  const updates = {
    Item: {
      "Flaming Sphere Damage": {
        "data.damage.parts": [[`${lastArg.spellLevel}d6`, "fire"]],
