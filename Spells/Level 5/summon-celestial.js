...
// DAE ON
if (args[0] == "on") {
  console.log("DAE ON: SUMMON CELESTIAL args: ", args)
}
// DAE itemMacro with spawned tokenId as argument
if (args[0] == "off") {
  console.log("DAE OFF: SUMMON CELESTIAL args: ", args)
...
if (lastArg.macroPass === "postActiveEffects") {
  console.log("MIDI: SUMMON CELESTIAL args: ", args)
  const spellSlot = lastArg.castData.castLevel; // base 5, above scales
  ...
  // Summon the token
  console.log(`await warpgate.spawn( ${choice.actor.name}, ${updates}, {}, ${options}))[0]}`);
  const celestialTokenId = (await warpgate.spawn(choice.actor.name, updates, {}, options))[0];
