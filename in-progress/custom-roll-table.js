...
const pack = game.packs.get("compendium-peashooters-ddb.tables");
const entry = pack.index.getName('Wild Magic Surge: Effect');
const tableUuid = entry.uuid;
const wmseTable = await fromUuid(tableUuid);
const wmseDraw = await wmseTable.draw({displayChat: false});
...
const draws = [{draw: wmseDraw, table: wmseTable}];
